...
import { VCard, VCardText, VCardTitle, VCheckbox, VCol, VDivider, VRow, VSwitch, VTextField } from 'vuetify/components'
...
// When enabled toggle changes, emit updated value
watch(enabled, (on) => {
  if (on) {
    // Activating: use current local values (or defaults)
    if (localWeekdays.value.length === 0) {
      localWeekdays.value = [0, 1, 2, 3, 4, 5, 6] // default: all days
    }
...
function onWeekdayToggle(day: number) {
  ...
  // If weekdays becomes empty, treat as disabled (emit null)
  if (localWeekdays.value.length === 0) {
    enabled.value = false
    emit('update:modelValue', null)
  } else {
...
const scheduleSummary = computed(() => {
  if (!enabled.value || !props.modelValue) return 'No schedule configured'
  const { startTime, endTime, weekdays } = props.modelValue
  const dayLabels = weekdays.map(d => WEEKDAY_LABELS.find(w => w.value === d)?.label).filter(Boolean)
  const crossesMidnight = startTime > endTime
  return `${startTime} – ${endTime}${crossesMidnight ? ' (next day)' : ''}, ${dayLabels.length === 7 ? 'Every day' : dayLabels.join(', ')}`
})
</script>
